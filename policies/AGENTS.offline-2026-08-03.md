...
This repository is an offline development and evaluation environment for the
Schlenker GX Works2 project. Use the official Computer Use plugin for GX Works2
GUI actions and ordinary shell/file tools for repository work.

## Mandatory safety gate

Before every GX Works2 UI run, obtain a current operator confirmation that no
PLC or production equipment is connected. A confirmation from an older run is
context only and does not satisfy a new run.
...
- Never select or approve Online, Monitor, Write to PLC, Read from PLC,
  Transfer, Remote Operation, diagnostics requiring a connection, or any saved
  connection destination.
- If GX Works2 displays an online/monitor/transfer/connection dialog or an
  unexpected target, stop without interacting and report the visible state.
...
- The same current-run disconnection confirmation is required before launching
  GX Works2 or taking a hybrid capture.
